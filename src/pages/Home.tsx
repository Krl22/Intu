...
const [routeDurationSeconds, setRouteDurationSeconds] = useState<number | null>(null);
  ...
  const [drawerDestinationLabel, setDrawerDestinationLabel] =
    useState<string>("");
  const [fitRoutePadding, setFitRoutePadding] = useState<{
    top?: number;
    right?: number;
    bottom?: number;
    left?: number;
  ...
  const NAV_HEIGHT = 64; // Altura de la barra inferior
  const MAP_BOTTOM_GAP = 40; // Margen extra para que el punto sur no quede tapado
  const SEARCH_BOTTOM_GAP = 56; // Extra adicional durante búsqueda para mostrar más hacia abajo
  ...
  useEffect(() => {
    if (!showFaresDrawer) {
      const extra = isSearchingDriver ? SEARCH_BOTTOM_GAP : MAP_BOTTOM_GAP;
      setFitRoutePadding((p) => ({ ...p, bottom: NAV_HEIGHT + extra }));
    }
        ...
        try {
          ...
          const response = await fetch(
            `/api/nominatim/search?format=json&q=${encodeURIComponent(
              ...
              lon + 0.1
            },${lat - 0.1}&bounded=1&countrycodes=us`
          );
  ...
  const handleConfirmMapSelection = async () => {
    if (mapCenter && userLocation) {
      const destinationText = `Lat: ${mapCenter.lat.toFixed(
        4
      )}, Lng: ${mapCenter.lng.toFixed(4)}`;
      setDrawerDestinationLabel(destinationText);
      setDestinationLocation([mapCenter.lat, mapCenter.lng]);
      setIsSelectingDestination(false);
      setIsSearchFocused(false);
  ...
  const handleDrawerVisibleHeight = (height: number) => {
    setFitRoutePadding({
      top: 24,
      right: 24,
      bottom: Math.max(0, height) + MAP_BOTTOM_GAP,
      left: 24,
    });
  };
  ...
  const handleCancelSearch = () => {
    ...
    setShowFaresDrawer(true);
    // Mantener un margen seguro mientras reaparece el drawer
    setFitRoutePadding({
      top: 24,
      right: 24,
      bottom: NAV_HEIGHT + MAP_BOTTOM_GAP,
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-200">
              ...
              <div className="mt-4 flex items-center">
                ...
                <p className="text-sm text-gray-700">
                  {searchPhases[searchPhaseIndex]}
                </p>
              </div>
              {searchVehicle && (
                <p className="mt-2 text-xs text-gray-500">
                  Tarifa estimada: ${searchVehicle.price}
                </p>
              )}
              <Button
      ...
      <FaresDrawer
        ...
        onConfirm={(vehicleType, estimatedPrice) => {
          ...
          setFitRoutePadding({
            top: 24,
            right: 24,
            bottom: NAV_HEIGHT + SEARCH_BOTTOM_GAP,
            left: 24,
          });
        }}
